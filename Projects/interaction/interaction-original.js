...
// When any key is pressed, fade out the background color first, then fade out the title
document.addEventListener('keydown', function () {
    const myBox = document.getElementById('myTitle');
    ...
    if (myBox) {
        ...
        // Use setTimeout to set the final background color after its transition completes
        setTimeout(function () {
            body.style.backgroundColor = 'white';
        }, 100);

        // Listen for the transitionend event on the body to trigger the title fade-out
        body.addEventListener('transitionend', function () {
            myBox.style.transition = 'opacity 0.5s ease';
            myBox.style.opacity = 0;

            // Use setTimeout to remove the title after its transition completes
            setTimeout(function () {
                myBox.remove();
            }, 100);
        }, { once: true });
