document.addEventListener('DOMContentLoaded', function () {
    ...
    // Create a listener to capture keystroke or touch events
    function handleInput(event) {
        ...
        // If the user pressed the space bar, add a blank space
        else if (event.key == ' ') {
    ...
    document.addEventListener('touchend', function (event) {
        ...
        if (Math.abs(swipeDistance) > 50) {
            ...
            if (swipeDistance > 0) {
                ...
                // Your swipe right handling code here
            } else {
                console.log('Swiped left');
                // Your swipe left handling code here
            }
        }
    });

    // When a letter key, spacebar, enter key, number key are touched, remove the background image/title page
